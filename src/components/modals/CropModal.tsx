...
import { Button, Typo, Theme, IconButton } from './ui/DesignSystem';
      ...
      const handleMouseMove = (e: MouseEvent) => {
          ...
          if (dragMode === 'move') {
              ...
              if (dragMode.includes('n')) {
                  const newHeight = Math.max(startCrop.height - deltaY, MIN_SIZE);
                  const newY = Math.min(Math.max(startCrop.y + deltaY, 0), startCrop.y + startCrop.height - MIN_SIZE);
                  // Only update if we aren't pushing top out of bounds or inverting
                  if (newY >= 0) {
                      newCrop.y = newY;
              ...
              if (dragMode.includes('w')) {
                  const newWidth = Math.max(startCrop.width - deltaX, MIN_SIZE);
