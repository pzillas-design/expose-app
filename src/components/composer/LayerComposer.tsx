    ...
    return (
        <div className="fixed inset-0 z-[100] flex flex-col overflow-hidden bg-zinc-50 dark:bg-black animate-in fade-in duration-150">
            ...
            <div className="relative flex-1 min-w-0 min-h-0 flex items-center justify-center p-4 md:p-8">
                ...
                />
                <BrushCursor canvasRef={canvasRef} brushSize={comp.brushSize} refW={comp.refDims.w} innerRatio={innerRatio} enabled={!!comp.activeId} hidden={isAdjusting} />
...
const BrushCursor: React.FC<{
    ...
    enabled: boolean;
    hidden?: boolean;
}> = ({ canvasRef, brushSize, refW, innerRatio, enabled, hidden }) => {
    ...
    return (
        <div className="pointer-events-none fixed z-[110]" style={{ left: state.x, top: state.y, transform: 'translate(-50%, -50%)' }}>
            <BrushRing size={state.size} innerRatio={innerRatio} />
        </div>
    );
};
