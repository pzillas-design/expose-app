...
export const AdminJobDetail: React.FC<AdminJobDetailProps> = ({ job, onClose, t, variant = 'sidebar' }) => {
 ...
 const providerRows: { label: string; value: React.ReactNode }[] = [
  ...
  ...((isFalNb2 || isOpenAI) && payload.endpoint ? [{ label: 'fal Endpoint', value: <span className="font-mono text-xs">{payload.endpoint}</span> }] : []),
  ...((isFalNb2 || isOpenAI) && payload.falRequestId ? [{ label: 'fal Request ID', value: <span className="font-mono text-xs">{payload.falRequestId}</span> }] : []),
  { label: 'Provider Response ID', value: formatValue(webhookData.providerResponseId || payload.responseId || ((isFalNb2 || isOpenAI) ? payload.falRequestId : null)) },
