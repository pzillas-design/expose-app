            ...
            <div className="max-w-7xl mx-auto w-full px-8">
                ...
                <header className="w-full pt-12 pb-8 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        />
                        <div className={`w-px h-4 ${Theme.Colors.BorderSubtle} mx-1`} />
                        <div className="flex flex-col items-end mr-1">
                            <span className={Typo.H3}>{userProfile?.full_name || user?.email?.split('@')[0]}</span>
                            <span className={Typo.Micro}>{user?.email}</span>
                        </div>
                        <button
                            onClick={() => onOpenSettings('account')}
                            className={`
                                w-10 h-10 rounded-full border border-zinc-200 dark:border-zinc-800 
                                flex items-center justify-center bg-white dark:bg-zinc-800 
                                hover:ring-2 ring-zinc-500/20 transition-all font-sans
                            `}
                        >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        ...
                        <button
                            ...
                            className={`
                                ...
                                hover:bg-zinc-50 dark:hover:bg-zinc-800/50 shadow-sm hover:shadow-md
                                min-h-[320px]
                            `}
                        >
...
function BoardCard({ board, onSelect, onDelete, onRename, locale }: BoardCardProps) {
    ...
    return (
        <Card
            className="group relative flex flex-col hover:shadow-md hover:-translate-y-1 transition-all duration-300 cursor-pointer border-zinc-200/60 dark:border-zinc-800/60"
            onClick={onSelect}
        >
            {/* Thumbnail Area */}
            <div className="aspect-[4/3] bg-zinc-100 dark:bg-zinc-800 relative flex items-center justify-center overflow-visible">
                {board.thumbnail ? (
                    <img src={board.thumbnail} alt={board.name} className="w-full h-full object-cover rounded-t-lg" />
                ) : (
                        ...
                        {isMenuOpen && (
                            <div className="absolute right-0 top-10 w-48 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-2xl z-20 py-1 animate-in fade-in zoom-in-95 duration-100">
                                <button
                                    ...
                                    className="w-full px-3 py-2 flex items-center gap-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-sm text-left font-sans"
                                >
                                    <Edit3 className="w-4 h-4" /> Umbenennen
                                </button>
                                <button
                                    onClick={() => { onDelete(); setIsMenuOpen(false); }}
                                    className="w-full px-3 py-2 flex items-center gap-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-sm text-red-500 text-left font-sans"
                                >
