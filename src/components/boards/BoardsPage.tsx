...
import { Plus, Trash, Pen, Clock, Image as ImageIcon, Settings, Wallet, Upload } from 'lucide-react';
...
export function BoardsPage({
    ...
    // Counter trick to handle nested drag events reliably
    ...
    const getPageStyles = () => {
        return 'bg-zinc-50 dark:bg-black text-zinc-900 dark:text-zinc-100 selection:bg-zinc-900 selection:text-white dark:selection:bg-white dark:selection:text-zinc-900';
    };

    return (
        <div
            className={`min-h-screen w-full flex flex-col transition-all duration-700 ${getPageStyles()} relative`}
        ...
        >
            ...
            {isDragOver && (
                <div
                    className={`fixed inset-0 z-[200] flex items-center justify-center transition-all duration-200 pointer-events-none
                        ${isDropActive ? 'bg-zinc-900/80' : 'bg-zinc-900/60'} backdrop-blur-sm`}
                >
                    <div
                        className={`flex flex-col items-center gap-4 p-12 rounded-3xl border-2 border-dashed transition-all duration-200
                            ${isDropActive ? 'border-white scale-105' : 'border-zinc-400 scale-100'}`}
                    >
                        <Upload className={`w-10 h-10 transition-colors ${isDropActive ? 'text-white' : 'text-zinc-300'}`} strokeWidth={1.5} />
                        <span className={`text-sm font-medium tracking-widest uppercase transition-colors ${isDropActive ? 'text-white' : 'text-zinc-300'}`}>
                            {lang === 'de' ? 'Bild in neues Projekt ablegen' : 'Drop image into new project'}
                        </span>
                    </div>
                </div>
            )}
