
export type LocaleKey = 'en' | 'de';

export const translations = {
  en: {
    // General
    cancel: "Cancel",
    save: "Save",
    delete: "Delete",
    edit: "Edit",
    create: "Create",
    saving: "Saving...",
    back: "Back",
    next: "Next",
    previous: "Previous",
    done: "Done",
    close: "Close",
    search: "Search...",
    copy: "Copy",
    copied_to_clipboard: "Prompt copied",
    back_to_app: "Back to App",
    home: "Home",
    reset_view: "Reset View",
    added: "added",
    attach_file: "Attach photo",
    new_generation: "New Generation",
    price_free: "Free",
    model_prefix: "Model: ",
    annotate: "Annotation",
    upload_failed: "Upload failed",
    image_ref: "Reference image",
    icon_label: "Icon / Symbol",
    name_de: "Name (DE)",
    name_en: "Name (EN)",
    actions: "Actions",
    beta_version_notice: "Looking for the old version? Visit",
    beta_notice_text: "Looking for the old version? Visit",
    credits_added_success: "added",

    // Error Messages
    error_no_input: "Please add a prompt first.",
    error_generation_failed: "Generation failed",
    error_insufficient_credits: "Insufficient credits",
    error_safety_blocked: "Content blocked by safety filters",
    error_invalid_prompt: "Invalid prompt",
    error_prompt_required: "Please enter a prompt or attach an image.",
    error_network_error: "Network error — please try again",
    error_server_error: "Server unavailable — please try again",
    error_unknown: "An unknown error occurred",
    error_session_expired: "Login expired — Please sign in again",
    error_session_invalid: "Session error — Please refresh",
    error_not_logged_in: "Not logged in",
    error_generation_timeout: "Generation timed out — credits refunded",
    error_network_timeout: "Connection too slow — Please try again",
    error_server_busy: "Google AI is currently experiencing high demand. Your credits have been refunded. Please try again shortly.",
    warning_google_overload: "High demand on Google AI. Please wait a moment.",
    error_file_type: "Format not supported — Please use JPG or PNG",
    error_cold_start: "Server cold start — Please retry",
    nav_gallery: "Gallery",
    generation_done_show: "Image ready — tap to view",
    generation_failed_title: "Generation failed",
    generation_failed_desc: "{{amount}} refunded.",
    retry: "Try again",
    select_image: "Mark image",
    deselect_image: "Unmark image",
    options: "Options",


    // SideSheet Header
    no_image: "No Image Selected",
    brush_tool: "Brush Tool",
    selection_tool: "Selection",
    objects_tool: "Objects",
    shapes_label: "Shapes",
    utilities_label: "Utilities",
    tools_label: "Tools",
    polygon_tool: "Polygon",
    batch_tool: "Batch",
    box_stamp: "Box",
    circle_stamp: "Circle",
    remove_label: "Remove",
    annotations_summary: "{{count}} Annotations",

    // Stickers (formerly Stamps)
    stamps_label: "Stickers",
    no_stamps: "No stickers found",
    enter_sticker_name: "Enter name...",
    add_sticker: "Insert",

    // Prompt Tab
    prompt_label: "Prompt",
    context_label: "Context",
    annotations: "Annotations",
    reset_tooltip: "Reset Preset",
    describe_changes: "Describe changes...",
    generate: "Generate",
    processing: "GENERATING...",
    tt_prompt: "Prompt",
    tt_generate: "Generate",
    tt_model: "Model",

    // Tools
    mask_btn: "Mask",
    object_btn: "Objects",
    image_btn: "Image",
    tt_annotate: "Draw comments",
    tt_upload_ref: "Add reference image",
    tool_disabled_multi: "Not available for multi-selection",

    // Presets
    no_presets: "No presets found",
    presets_header: "Presets",
    search_presets: "Search presets...",
    create_preset: "Create Preset",
    update_preset: "Update Preset",
    delete_preset: "Delete",
    confirm_delete_preset: "Delete this preset?",
    preset_name_label: "Name",
    preset_prompt_label: "Prompt",
    preset_created: "Preset created",
    preset_updated: "Preset updated",
    preset_deleted: "Preset deleted",
    new_preset_title: "Create New Preset",
    edit_preset_title: "Edit Preset",
    title_label: "Title",
    title_placeholder: "e.g. Cinematic Light",
    prompt_label_editor: "Prompt",
    prompt_placeholder: "e.g. add cinematic lighting",
    variables_label: "Variables",
    add_variable: "Add Variable",
    version_de: "German Version",
    version_en: "English Version",
    var_name_placeholder: "e.g. Mood",
    var_options_placeholder: "e.g. warm, neutral (comma separated)...",
    add_btn: "Add",

    // Variables
    vars_label: "Variables",
    add_var: "Add Var",
    var_placeholder: "Value...",

    // Help & Tooltips
    brush_hint: "Paint over areas you want to change.",
    object_hint: "Place objects as references.",
    tt_deselect: "Deselect",
    tt_clear: "Clear All",
    tt_download: "Download",
    tt_delete: "Delete",
    tt_undo: "Undo",
    tt_redo: "Redo",
    zoom_in: "Zoom In",
    zoom_out: "Zoom Out",
    manage_credits: "Manage Credits",
    create_new: "Create New",

    // Board / Gallery
    welcome_title: "Welcome to Exposé",
    welcome_desc: "Select your photos to edit or start a new flow.",
    welcome_empty_desc: "Upload your photos or generate something new.",
    empty_board_title: "Your board is empty",
    empty_board_desc: "Upload images or use the generator to get started.",
    upload_image_edit: "Upload Image & Edit",
    generate_new: "Generate new image",
    empty_feed_message: "Click \"+\" to generate your first image.",
    action_upload: "Upload",
    drop_files_here: "Drop files here",
    action_generate: "Generate image",

    // Settings / User
    login_btn: "Login",
    logout_btn: "Logout",
    credits_label: "Credits",
    billing_btn: "Billing",
    admin_panel: "Admin",

    // Image Context Menu
    ctx_edit: "Edit",
    ctx_copy_prompt: "Copy Prompt",
    copy_image: "Copy",
    ctx_download: "Download",
    ctx_delete_image: "Delete",
    ctx_delete: "Delete",
    ctx_set_reference: "Set as Reference",

    // Multi Select & Context Menu
    images_selected: "images",
    delete_confirm_multi: "Are you sure you want to delete these images?",
    delete_confirm_single: "Are you sure you want to delete this image?",
    download_all: "Download All",
    delete_all: "Delete All",
    generate_multi: "Generate",
    generate_multiple_title: "Do you want to generate {{count}} images?",
    mixed_prompts: "Mixed Prompts",
    ctx_select_all: "Select All",
    ctx_deselect: "Deselect",
    ctx_delete_selection: "Delete Selection",
    ctx_download_selection: "Download Selection",
    ctx_download_multi: "Save {{n}} images",
    ctx_delete_multi: "Delete {{n}} images",
    ctx_download_image: "Save",

    // Generation Modal
    gen_modal_title: "Generate Image",
    gen_modal_prompt_placeholder: "A futuristic city at night...",
    gen_modal_submit: "Generate",
    gen_modal_aspect_ratio: "Aspect Ratio",
    gen_modal_quality: "Quality",
    creation_prompt_label: "Prompt",
    creation_prompt_placeholder: "Describe what you want to generate...",
    creation_quality_label: "Quality",
    creation_ratio_label: "Aspect Ratio",
    drop_here: "Drop here",

    // Admin Panel
    admin_users: "Users",
    admin_presets: "Presets",
    admin_objects: "Stickers",
    admin_settings: "Settings",
    tab_users: "Users",
    tab_presets: "Presets",
    tab_objects: "Stickers",
    tab_edit: "Edit",
    resolution: "Resolution",
    model: "Model",
    created_at: "Created",
    no_mask_found: "No mask found for this image",
    debug_mask: "Debug: Check Mask",
    admin_user_updated_success: "User updated successfully",
    failed_update_user: "Failed to update user",
    failed_delete_user: "Failed to delete user",
    role_admin: "Admin",
    id_label: "ID",
    tab_info: "Info",
    ctx_create_variations: "Generate More",
    filename: "Filename",
    reference_images: "Reference Images",
    info: "Info",
    tt_more: "More options",
    tt_navigate_parent: "Navigate to parent image",
    tt_generate_more: "Generate more variations",
    tt_save: "Download image",
    image_info: "Image Info",
    dimensions: "Dimensions",
    quality: "Quality",
    created: "Created",
    prompt: "Prompt",
    user_prompt: "User Prompt",
    parent_image: "Parent Image",

    // Auth
    auth_email_placeholder: "Email address",
    auth_password_placeholder: "Password",
    auth_new_password_placeholder: "New password",
    auth_confirm_password_placeholder: "Confirm password",
    auth_sign_in_btn: "Sign In",
    auth_create_account: "Create Account",
    auth_create_account_btn: "Create Account",
    auth_reset_password: "Reset Password",
    auth_update_password_title: "Set New Password",
    auth_update_password_btn: "Set New Password",
    auth_send_reset_link: "Send Reset Link",
    auth_or_continue: "Or continue with",
    auth_google: "Google Login",
    auth_no_account: "Create account",
    auth_forgot_password: "Forgot password?",
    auth_already_have: "Already have an account?",
    auth_back_to_signin: "Back to sign in",
    auth_signup_desc: "Get started with your new account",
    auth_reset_desc: "Enter your email to reset your password",
    auth_check_email_conf: "Check your email to confirm your account",
    auth_check_email_reset: "Check your email for a password reset link",
    auth_error_link_expired: "This reset link has expired or was already used. Please request a new one below.",
    auth_confirm_desc: "For your security, please confirm with a click that you opened this link yourself.",
    auth_confirm_btn: "Continue",
    auth_confirm_verifying: "One moment…",
    auth_confirm_request_new: "Request a new link",
    auth_confirm_success_desc: "Your password has been updated. You can now sign in.",
    auth_password_updated: "Password updated successfully",
    auth_resend_link: "Resend",
    auth_error_password_same: "New password must be different from the old one",
    auth_error_password_mismatch: "Passwords do not match",
    auth_error_invalid_credentials: "Invalid email or password. If you signed up with Google, use \"Continue with Google\" below — that account has no password.",
    auth_error_user_exists: "An account with this email already exists",
    auth_error_invalid_link: "Invalid or expired link",
    auth_error_schema: "Authentication is temporarily unavailable due to a server configuration issue.",
    tab_account: "Account",
    tab_general: "General",
    tab_about: "Home",
    admin_dashboard: "Admin Dashboard",
    admin_jobs: "Jobs",
    admin_job_user: "User",
    admin_job_status: "Status",
    admin_job_quality: "Quality",
    admin_job_cost: "Cost",
    admin_job_date: "Date",
    admin_job_completed: "Completed",
    admin_job_failed: "Failed",
    admin_job_processing: "Processing",
    admin_job_details: "Job Details",
    admin_job_prompt: "Prompt",
    admin_job_api_cost: "API Cost",
    admin_financials: "Financials",
    admin_timestamps: "Timestamps",
    admin_resources_used: "Resources Used",
    admin_user_details: "User Details",
    admin_total_spent: "Total Spent",
    admin_balance: "Balance",
    admin_add_funds: "Add Funds",
    admin_account_section: "Account",
    admin_role_label: "Role",
    admin_user_name: "Name",
    admin_user_email: "Email",
    admin_last_online: "Last Online",
    admin_user_joined: "Joined",
    admin_delete_user: "Delete User",
    admin_delete_user_desc: "Do you really want to permanently delete this user?",
    admin_delete_user_confirm: "Delete {{name}}?",
    admin_user_deleted_success: "User deleted successfully",
    admin_add_credits_success: "{{amount}} credits added successfully",
    admin_pass_reset_success: "Password reset email sent to {{email}}",
    admin_pass_reset_error: "Failed to send password reset email",
    admin_just_now: "Just now",
    admin_mins_ago: "{{n}} mins ago",
    admin_hours_ago: "{{n}} hours ago",
    admin_days_ago: "{{n}} days ago",
    sign_out: "Sign Out",
    balance: "Balance",
    top_up: "Top Up",
    app_section: "Design Theme",
    lang_section: "Language",
    mode_light: "Light",
    mode_dark: "Dark",
    mode_system: "System",
    tab_projects: "Projects",
    tab_settings: "Settings",
    overview_desc: "Overview of all your projects.",
    rename_board: "Rename",
    default_project_name: "New Project",
    footer_about: "About",
    footer_blog: "Blog",
    footer_website: "Website",
    footer_contact: "Contact",
    footer_legal: "Legal",
    footer_privacy: "Privacy",
    footer_photos: "Gallery",
    menu_photos: "Gallery",
    menu_select: "Select",
    // New
    account_deleted: "Account deleted successfully",
    admin_danger_zone: "Danger Zone",
    admin_reset_password: "Reset Password",
    checkout_btn: "Checkout",
    checkout_min_amount: "Minimum amount: 5€",
    checkout_pay: "Pay Now",
    ctx_add_to_selection: "Mark",
    ctx_remove_from_selection: "Unmark",
    current_balance: "Current Balance",
    delete_failed: "Deletion failed",
    failed_create_board: "Failed to create project",
    failed_delete_account: "Failed to delete account",
    failed_delete_board: "Failed to delete project",
    failed_load_boards: "Failed to load projects",
    failed_update_board: "Failed to update project",
    failed_update_profile: "Failed to update profile",
    finishing: "Finishing...",
    generation_failed: "Generation failed",
    no_entries_found: "No entries found",
    payment_failed: "Payment failed",
    payment_success: "Payment successful",
    profile_updated: "Profile updated successfully",
    role_pro: "Pro",
    role_user: "User",
    save_failed: "Save failed",
    settings_email_label: "Email Address",
    settings_delete_account: "Delete Account",
    settings_delete_account_desc: "Do you really want to permanently delete your account?",
    admin_job_type: "Type",
    admin_job_edit: "Edit",
    admin_job_create: "Gen.",
    admin_technical_details: "Technical Details",
    admin_source_image: "Source Image",
    admin_mask: "Mask",
    yes: "Yes",
    no: "No",
    admin_jobs_desc: "History of all generation processes and API calls.",
    load_more: "Load more",
    more: "More",
    save_error: "Error while saving",
    delete_error: "Error while deleting",
    credits_custom_amount: "Enter a different amount",
    credits_payment_note: "Credits are available immediately after payment.",
    save_annotations: "Save annotations?",
    save_discard_changes: "Save or discard changes?",
    discard: "Discard",
    save_success: "Successfully saved",
    delete_success: "Successfully deleted",
    admin_total_spent_header: 'Total Spent',
    admin_user_default: 'User',
    admin_user_email_missing: 'No Email',
    delete_account_section: 'Other',
    delete_account_permanently: 'Delete Account',
    shared_template_label: "Shared Template",
    shared_template_desc: "Use this exclusive template to take your images to the next level directly in exposé.",
    shared_template_cta: "Add Template",
    shared_template_not_found: "Template not found",
    shared_template_invalid: "This link seems to be invalid or the template has been removed.",
    back_to_home: "Go to homepage",
    learn_more_expose: "Learn more about exposé",

    // AppNavbar selection count
    image_selected_singular: "image selected",
    images_selected_plural: "images selected",

    // GenerationProgressRing
    untitled: "Untitled",
    go_to_image: "Go to image",
    generate_more: "Generate more",

    // SideSheet annotation chips
    annotations_label: "Annotations",
    brush_label: "Brush",
    shape_rect: "Rect",
    shape_circle: "Circle",
    shape_line: "Line",
    stamp_label: "Stamp",
    add_reference: "Add Inspiration",
    quality_highest: "Highest quality",
    quality_faster: "Faster & cheaper",
    presets_label: "Presets",
    no_entries: "No entries",
    edit_presets: "Edit Presets",

    // CropModal
    crop_title: "Crop",
    crop_subtitle: "Adjust the image section.",

    // FeedPage
    select_images_to_edit: "Select images to edit",
    n_download: "Download {{n}}",

    // SettingsModal
    storage_section: "Storage",
    images_label: "images",
    notifications_label: "Notifications",
    notifications_active: "Triggered after each generation",
    notifications_blocked: "Blocked in browser",
    notifications_disabled: "Currently disabled",
    notifications_ask: "Permission will be requested when enabled",
    notifications_allow_in_browser: "Allow notifications in your browser settings",
    // SettingsModal — extended
    settings_model_label: "Default Model",
    settings_quality_label: "Quality",
    settings_change_password: "Change Password",
    settings_set_password: "Set a password (optional)",
    notifications_browser_label: "Browser Notifications",
    notifications_browser_desc: "After completed generations",
    notification_generation_done: "exposé · Image ready",
    model_nb2_desc: "Faster",
    model_pro_desc: "Highest quality",

    // DetailPage
    open_editing_panel: "Open editing panel",

    // BrushTab tool labels
    tool_text: "Text",
    tool_shape: "Shape",
    tool_circle: "Circle",
    tool_line: "Line",
    tool_brush: "Brush",
    clear_all_brush: "Clear brush strokes",
    tooltip_tool_text: "Text",
    tooltip_tool_shapes: "Shapes",
    tooltip_tool_brush: "Brush",

    // ObjectsTab
    manage_stickers: "Manage Stickers",
    add_new_sticker: "Add New Sticker",
    no_stickers_yet: "No stickers yet.",
    create_first_sticker: "Create your first sticker",
    edit_stickers_btn: "Edit Stickers",
    sticker_create_tooltip: "Create Sticker",

    // ImageInfoModal
    copy_prompt_tooltip: "Copy prompt",

    // PromptTab
    select_model: "Select model",
    please_enter_prompt: "Please enter a prompt.",
    image_still_uploading: "Image is still uploading — try again in a moment.",
    reference_image_btn: "Reference Image",
    annotation_instruction: "Interpret the visual annotations. They show what and where to change in the original image ..",
    reference_image_instruction: "Use this image as inspiration ..",
    clear_all_annotations: "Clear all annotations",
    clear_all_annotations_desc: "Do you really want to remove all annotations from the canvas? This action cannot be undone.",

    // AppNavbar
    nav_create: "New",
    nav_create_tooltip: "Upload or generate an image",
    nav_generate_new: "Generate new image",
    nav_upload: "Upload",
    nav_menu: "Menu",
    nav_start: "Start",
    nav_settings: "Settings",
    nav_select: "Select images",
    nav_contact: "Feedback",
    nav_sign_out: "Sign out",
    nav_logout: "Logout",
    nav_download: "Download",
    nav_more: "More",
    nav_sidebar: "Sidebar",
    nav_edit: "Edit",
    nav_delete: "Delete",
    nav_info: "Info",

    // PresetLibrary
    presets_recent: "Recent",
    presets_recently_used: "Recently used",
    presets_options: "Options",
    presets_edit_tooltip: "Edit template",
    preset_edit_presets: "Edit Presets",
    no_entries_available: "No entries available yet.",

    // AuthModal
    auth_or: "or",

    // ManagePresetsModal / ShareTemplateModal
    share_btn: "Share",
    share_title: "Share Template",
    share_ready_desc: "Your template is ready to share — copy the link and send it to friends or colleagues.",
    share_unsaved_desc: "Please save your changes first to generate a shareable link.",
    share_placeholder_unsaved: "Link will be generated after saving...",
    share_copy_btn: "Copy Link",
    share_copied_btn: "Link Copied",
    share_error_copy: "Error copying link",
    share_error_save_first: "Please save the template first",
    share_toast_copied: "Link copied",
    preset_select_placeholder: "Select a preset or create a new one",

    // Home Page
    home_hero_headline: "ready for next-gen creation?",
    home_section_iterative_title: "rapid iterations",
    home_section_iterative_desc: "Generate entire image series at once — compare, refine, and perfect them side by side. Powered by Google’s Nano Banana 2, the best AI image model available today.",
    home_section_templates_title: "custom templates",
    home_section_templates_desc: "Prompt once, scale forever. Build your own template library and generate consistent image assets at scale.",
    home_section_visual_title: "Visual Prompting",
    home_section_visual_desc: "Don't just tell the AI what to do — show it exactly where.",
    home_mockup_prompt_hint: "Reimagine this scene by changing the season",
    home_mockup_season: "Season",
    home_mockup_time: "Time of Day",
    home_mockup_generate: "Generate",
    home_mockup_templates: "Templates",
    home_mockup_note: "Note",
    home_mockup_ref: "Reference",
    home_spring: "Spring",
    home_summer: "Summer",
    home_winter: "Winter",
    home_morning: "Morning",
    home_noon: "Midday",
    home_afternoon: "Afternoon",
    home_golden_hour: "Golden Hour",
    home_blue_hour: "Blue Hour",
    home_kitchen: "Kitchen",
    home_table: "Dining Table",
    home_sofa: "Sofa",
  },
  de: {
    // General
    cancel: "Abbrechen",
    save: "Speichern",
    delete: "Löschen",
    edit: "Bearbeiten",
    create: "Erstellen",
    back: "Zurück",
    next: "Weiter",
    previous: "Zurück",
    done: "Fertig",
    close: "Schließen",
    search: "Suchen...",
    copy: "Kopieren",
    copied_to_clipboard: "Prompt kopiert",
    back_to_app: "Zurück zur App",
    home: "Home",
    reset_view: "Ansicht zurücksetzen",
    added: "hinzugefügt",
    attach_file: "Foto anhängen",
    new_generation: "Neue Generation",
    price_free: "Kostenlos",
    model_prefix: "Modell: ",
    annotate: "Anmerkung",
    upload_failed: "Upload fehlgeschlagen",
    image_ref: "Referenzbild",

    // SideSheet Header
    no_image: "Kein Bild ausgewählt",
    brush_tool: "Pinsel",
    selection_tool: "Auswahl",
    objects_tool: "Objekte",
    shapes_label: "Formen",
    utilities_label: "Hilfsmittel",
    tools_label: "Werkzeuge",
    polygon_tool: "Polygon",
    batch_tool: "Stapel",
    box_stamp: "Box",
    circle_stamp: "Kreis",
    remove_label: "Entfernen",
    icon_label: "Icon / Symbol",
    name_de: "Name (DE)",
    name_en: "Name (EN)",
    actions: "Aktionen",
    beta_version_notice: "Alte Version? Besuche",
    beta_notice_text: "Suchen Sie die alte Version? Besuchen Sie",
    credits_added_success: "aufgeladen",

    // Error Messages
    error_no_input: "Bitte zuerst einen Prompt hinzufügen.",
    error_generation_failed: "Generierung fehlgeschlagen",
    error_insufficient_credits: "Nicht genügend Credits",
    error_safety_blocked: "Inhalt durch Sicherheitsfilter blockiert",
    error_invalid_prompt: "Ungültiger Prompt",
    error_prompt_required: "Bitte einen Prompt eingeben oder ein Bild anhängen.",
    error_network_error: "Netzwerkfehler – bitte erneut versuchen",
    error_server_error: "Server nicht erreichbar – bitte erneut versuchen",
    error_unknown: "Ein unbekannter Fehler ist aufgetreten",
    error_session_expired: "Anmeldung abgelaufen – Bitte neu einloggen",
    error_session_invalid: "Sitzungs-Fehler – Bitte Seite neu laden",
    error_not_logged_in: "Nicht eingeloggt",
    error_generation_timeout: "Generierung abgebrochen – Credits erstattet",
    error_network_timeout: "Verbindung zu langsam – Bitte erneut versuchen",
    error_server_busy: "Google AI ist momentan überlastet. Credits wurden erstattet. Bitte versuchen Sie es in Kürze erneut.",
    warning_google_overload: "Erhöhte Nachfrage bei Google AI. Bitte einen Moment warten.",
    error_file_type: "Format wird nicht unterstützt – Bitte JPG oder PNG nutzen",
    error_cold_start: "Server-Kaltstart – bitte erneut versuchen",
    generation_done_show: "Bild fertig — zum Ansehen tippen",
    generation_failed_title: "Generierung fehlgeschlagen",
    generation_failed_desc: "{{amount}} erstattet.",
    retry: "Nochmal versuchen",
    select_image: "Bild markieren",
    deselect_image: "Markierung aufheben",
    options: "Optionen",
    annotations_summary: "{{count}} Anmerkungen",

    // Stickers (früher Stempel)
    stamps_label: "Sticker",
    no_stamps: "Keine Sticker gefunden",
    enter_sticker_name: "Name eingeben...",
    add_sticker: "Einfügen",
    admin_objects_desc: "Verwalte die Sticker und Symbole für das Anmerkungs-Werkzeug.",
    delete_selected: "Auswahl löschen ({{count}})",
    add_stamp: "Stempel hinzufügen",

    // Prompt Tab
    prompt_label: "PROMPT",
    context_label: "KONTEXT",
    annotations: "Anmerkungen",
    reset_tooltip: "Preset zurücksetzen",
    describe_changes: "Änderungen beschreiben...",
    generate: "Generieren",
    processing: "GENERIEREN...",
    tt_prompt: "Prompt",
    tt_generate: "Generieren",
    tt_model: "Modell",

    // Tools
    mask_btn: "Maske",
    object_btn: "Objekte",
    image_btn: "Bild",
    tt_annotate: "Kommentare zeichnen",
    tt_upload_ref: "Referenzbild hinzufügen",
    tool_disabled_multi: "Nicht verfügbar bei Mehrfachauswahl",

    // Presets
    no_presets: "Keine Presets gefunden",
    presets_header: "Vorlagen",
    search_presets: "Presets suchen...",
    create_preset: "Preset erstellen",
    update_preset: "Preset aktualisieren",
    delete_preset: "Löschen",
    saving: "Speichern...",
    confirm_delete_preset: "Dieses Preset löschen?",
    preset_name_label: "Name",
    preset_prompt_label: "Prompt",
    preset_created: "Preset erstellt",
    preset_updated: "Preset aktualisiert",
    preset_deleted: "Preset gelöscht",
    new_preset_title: "Neue Vorlage erstellen",
    edit_preset_title: "Vorlage bearbeiten",
    title_label: "Titel",
    title_placeholder: "z.B. Cinematic Light",
    prompt_label_editor: "Prompt",
    prompt_placeholder: "z.B. füge cinematisches Licht hinzu",
    variables_label: "Variablen",
    add_variable: "Variable hinzufügen",
    version_de: "Deutsche Version",
    version_en: "Englische Version",
    var_name_placeholder: "z.B. Stimmung",
    var_options_placeholder: "z.B. warm, neutral (durch Komma getrennt)",
    add_btn: "Hinzufügen",

    // Variables
    vars_label: "Variablen",
    add_var: "Variable",
    var_placeholder: "Wert...",

    // Help & Tooltips
    brush_hint: "Übermalen Sie Bereiche, die Sie ändern möchten.",
    object_hint: "Platzieren Sie Objekte als Referenz.",
    tt_deselect: "Auswahl aufheben",
    tt_clear: "Alles löschen",
    tt_download: "Herunterladen",
    tt_delete: "Löschen",
    tt_undo: "Rückgängig",
    tt_redo: "Wiederholen",
    zoom_in: "Vergrößern",
    zoom_out: "Verkleinern",
    manage_credits: "Guthaben verwalten",
    create_new: "Neu erstellen",

    // Board / Gallery
    welcome_title: "Willkommen bei Exposé",
    welcome_desc: "Fotos auswählen oder generieren Sie etwas Neues.",
    welcome_empty_desc: "Laden Sie Fotos zum Bearbeiten hoch oder generieren Sie etwas Neues.",
    empty_board_title: "Ihr Board ist leer",
    empty_board_desc: "Lade Bilder hoch oder nutze den Generator, um zu starten.",
    upload_image_edit: "Hochladen",
    generate_new: "Erstellen",
    empty_feed_message: "Über \"+\" erstellen Sie Ihr erstes Bild.",
    action_upload: "Hochladen",
    drop_files_here: "Dateien hier ablegen",
    action_generate: "Erstellen",

    // Settings / User
    login_btn: "Anmelden",
    logout_btn: "Abmelden",
    credits_label: "Credits",
    billing_btn: "Abrechnung",
    admin_panel: "Admin",

    // Image Context Menu
    ctx_edit: "Bearbeiten",
    ctx_copy_prompt: "Prompt kopieren",
    copy_image: "Kopieren",
    ctx_download: "Herunterladen",
    ctx_delete_image: "Löschen",
    ctx_delete: "Löschen",
    ctx_set_reference: "Als Referenz setzen",

    // Multi Select & Context Menu
    images_selected: "Bilder",
    delete_confirm_multi: "Möchtest du diese Bilder wirklich löschen?",
    delete_confirm_single: "Möchtest du dieses Bild wirklich löschen?",
    download_all: "Alle herunterladen",
    delete_all: "Alle löschen",
    generate_multi: "Generieren",
    generate_multiple_title: "Möchten Sie {{count}} Bilder generieren?",
    mixed_prompts: "Gemischte Prompts",
    ctx_select_all: "Alles auswählen",
    ctx_deselect: "Auswahl aufheben",
    ctx_delete_selection: "Auswahl löschen",
    ctx_download_selection: "Auswahl herunterladen",
    ctx_download_multi: "{{n}} Bilder speichern",
    ctx_delete_multi: "{{n}} Bilder löschen",
    ctx_download_image: "Speichern",

    // Generation Modal
    gen_modal_title: "Bild generieren",
    gen_modal_prompt_placeholder: "Eine futuristische Stadt bei Nacht...",
    gen_modal_submit: "Generieren",
    gen_modal_aspect_ratio: "Format",
    gen_modal_quality: "Qualität",
    creation_prompt_label: "Prompt",
    creation_prompt_placeholder: "Beschreiben Sie, was Sie generieren möchten...",
    creation_quality_label: "Qualität",
    creation_ratio_label: "Format",
    drop_here: "Hier ablegen",

    // Admin Panel
    admin_users: "Benutzer",
    admin_presets: "Presets",
    admin_objects: "Sticker",
    admin_settings: "Einstellungen",
    tab_users: "Benutzer",
    tab_presets: "Presets",
    tab_objects: "Sticker",
    tab_edit: "Bearbeiten",
    resolution: "Auflösung",
    model: "Modell",
    created_at: "Erstellt",
    no_mask_found: "Keine Maske für dieses Bild gefunden",
    debug_mask: "Debug: Maske prüfen",
    admin_user_updated_success: "Benutzer erfolgreich aktualisiert",
    failed_update_user: "Benutzer konnte nicht aktualisiert werden",
    failed_delete_user: "Benutzer konnte nicht gelöscht werden",
    role_admin: "Admin",
    id_label: "ID",
    tab_info: "Info",
    ctx_create_variations: "Mehr generieren",
    filename: "Dateiname",
    reference_images: "Referenzbilder",
    info: "Info",
    tt_more: "Weitere Optionen",
    tt_navigate_parent: "Zum Ursprungsbild navigieren",
    tt_generate_more: "Mehr Variationen generieren",
    tt_save: "Bild herunterladen",
    image_info: "Bild-Info",
    dimensions: "Abmessungen",
    quality: "Qualität",
    created: "Erstellt",
    prompt: "Prompt",
    user_prompt: "Benutzer-Prompt",
    parent_image: "Ursprungsbild",

    // Auth
    auth_email_placeholder: "E-Mail-Adresse",
    auth_password_placeholder: "Passwort",
    auth_new_password_placeholder: "Neues Passwort",
    auth_confirm_password_placeholder: "Passwort bestätigen",
    auth_sign_in_btn: "Anmelden",
    auth_create_account: "Konto erstellen",
    auth_create_account_btn: "Konto erstellen",
    auth_reset_password: "Passwort zurücksetzen",
    auth_update_password_title: "Neues Passwort festlegen",
    auth_update_password_btn: "Neues Passwort festlegen",
    auth_send_reset_link: "Link senden",
    auth_or_continue: "Oder fortfahren mit",
    auth_google: "Google Login",
    auth_no_account: "Konto erstellen",
    auth_forgot_password: "Passwort vergessen?",
    auth_already_have: "Bereits ein Konto?",
    auth_back_to_signin: "Zurück zur Anmeldung",
    auth_signup_desc: "Legen Sie Ihr neues Konto an",
    auth_reset_desc: "Geben Sie Ihre E-Mail ein, um Ihr Passwort zurückzusetzen",
    auth_check_email_conf: "Prüfen Sie Ihre E-Mails zur Kontobestätigung",
    auth_check_email_reset: "Prüfen Sie Ihre E-Mails für einen Passwort-Reset-Link",
    auth_error_link_expired: "Dieser Link ist abgelaufen oder wurde bereits verwendet. Bitte fordern Sie unten einen neuen an.",
    auth_confirm_desc: "Bitte bestätigen Sie aus Sicherheitsgründen mit einem Klick, dass Sie diesen Link selbst geöffnet haben.",
    auth_confirm_btn: "Weiter",
    auth_confirm_verifying: "Einen Moment…",
    auth_confirm_request_new: "Neuen Link anfordern",
    auth_confirm_success_desc: "Ihr Passwort wurde geändert. Sie können sich jetzt anmelden.",
    auth_password_updated: "Passwort erfolgreich aktualisiert",
    auth_resend_link: "Erneut senden",
    auth_error_password_same: "Neues Passwort muss sich vom alten unterscheiden",
    auth_error_password_mismatch: "Passwörter stimmen nicht überein",
    auth_error_invalid_credentials: "Ungültige E-Mail oder Passwort. Falls Sie sich mit Google registriert haben, nutzen Sie unten „Mit Google anmelden“ — dieses Konto hat kein Passwort.",
    auth_error_user_exists: "Ein Konto mit dieser E-Mail existiert bereits",
    auth_error_invalid_link: "Ungültiger oder abgelaufener Link",
    auth_error_schema: "Anmeldung ist wegen eines Server-Konfigurationsproblems momentan nicht verfügbar.",
    tab_account: "Konto",
    tab_general: "Allgemein",
    tab_about: "Home",
    admin_dashboard: "Admin-Dashboard",
    admin_jobs: "Aufträge",
    admin_job_user: "Benutzer",
    admin_job_status: "Status",
    admin_job_quality: "Qualität",
    admin_job_cost: "Kosten",
    admin_job_date: "Datum",
    admin_job_completed: "Abgeschlossen",
    admin_job_failed: "Fehlgeschlagen",
    admin_job_processing: "In Bearbeitung",
    admin_job_details: "Auftrags-Details",
    admin_job_prompt: "Prompt",
    admin_job_api_cost: "API-Kosten",
    admin_financials: "Finanzen",
    admin_timestamps: "Zeitstempel",
    admin_resources_used: "Verwendete Ressourcen",
    admin_user_details: "Benutzer-Details",
    admin_total_spent: "Ausgaben gesamt",
    admin_balance: "Guthaben",
    admin_add_funds: "Guthaben aufladen",
    admin_account_section: "Konto",
    admin_role_label: "Rolle",
    admin_user_name: "Name",
    admin_user_email: "E-Mail",
    admin_last_online: "Zuletzt online",
    admin_user_joined: "Beigetreten",
    admin_delete_user: "Benutzer löschen",
    admin_delete_user_desc: "Möchtest du diesen Benutzer wirklich unwiderruflich löschen?",
    admin_delete_user_confirm: "{{name}} löschen?",
    admin_user_deleted_success: "Benutzer erfolgreich gelöscht",
    admin_add_credits_success: "{{amount}} gutgeschrieben",
    admin_pass_reset_success: "Passwort-Reset-E-Mail an {{email}} gesendet",
    admin_pass_reset_error: "Fehler beim Senden der Passwort-Reset-E-Mail",
    admin_just_now: "Gerade eben",
    admin_mins_ago: "Vor {{n}} Min.",
    admin_hours_ago: "Vor {{n}} Std.",
    admin_days_ago: "Vor {{n}} Tagen",
    sign_out: "Abmelden",
    balance: "Guthaben",
    top_up: "Aufladen",
    app_section: "Darstellung",
    lang_section: "Sprache",
    mode_light: "Hell",
    mode_dark: "Dunkel",
    mode_system: "System",
    tab_projects: "Projekte",
    tab_settings: "Einstellungen",
    overview_desc: "Überblick über alle Ihre Projekte.",
    rename_board: "Umbenennen",
    default_project_name: "Neues Projekt",
    footer_about: "About",
    footer_blog: "Blog",
    footer_website: "Website",
    footer_contact: "Kontakt",
    footer_legal: "Impressum",
    footer_privacy: "Datenschutz",
    footer_photos: "Galerie",
    menu_photos: "Galerie",
    menu_select: "Auswählen",
    // Neu
    account_deleted: "Konto erfolgreich gelöscht",
    admin_danger_zone: "Gefahrenzone",
    admin_reset_password: "Passwort zurücksetzen",
    checkout_btn: "Bezahlen",
    checkout_min_amount: "Mindestbetrag: 5€",
    checkout_pay: "Jetzt bezahlen",
    ctx_add_to_selection: "Markieren",
    ctx_remove_from_selection: "Markierung aufheben",
    current_balance: "Aktuelles Guthaben",
    delete_failed: "Löschen fehlgeschlagen",
    failed_create_board: "Projekt konnte nicht erstellt werden",
    failed_delete_account: "Konto konnte nicht gelöscht werden",
    failed_delete_board: "Projekt konnte nicht gelöscht werden",
    failed_load_boards: "Projekte konnten nicht geladen werden",
    failed_update_board: "Projekt konnte nicht aktualisiert werden",
    failed_update_profile: "Profil konnte nicht aktualisiert werden",
    finishing: "Wird abgeschlossen...",
    generation_failed: "Generierung fehlgeschlagen",
    no_entries_found: "Keine Einträge gefunden",
    payment_failed: "Zahlung fehlgeschlagen",
    payment_success: "Zahlung erfolgreich",
    profile_updated: "Profil erfolgreich aktualisiert",
    role_pro: "Pro",
    role_user: "Benutzer",
    save_failed: "Speichern fehlgeschlagen",
    settings_email_label: "E-Mail-Adresse",
    settings_delete_account: "Konto löschen",
    settings_delete_account_desc: "Möchten Sie Ihr Konto wirklich dauerhaft löschen?",
    admin_job_type: "Typ",
    admin_job_edit: "Bearbeitung",
    admin_job_create: "Erstellung",
    admin_technical_details: "Technische Details",
    admin_source_image: "Quellbild",
    admin_mask: "Maske",
    yes: "Ja",
    no: "Nein",
    admin_jobs_desc: "Historie aller Generierungsprozesse und API-Calls.",
    load_more: "Mehr laden",
    more: "Mehr",
    save_error: "Fehler beim Speichern",
    delete_error: "Fehler beim Löschen",
    credits_custom_amount: "Anderen Betrag eingeben",
    credits_payment_note: "Guthaben ist sofort nach Abschluss der Zahlung verfügbar.",
    save_annotations: "Anmerkungen speichern?",
    save_discard_changes: "Änderungen speichern oder verwerfen?",
    discard: "Verwerfen",
    save_success: "Erfolgreich gespeichert",
    delete_success: "Erfolgreich gelöscht",
    admin_total_spent_header: 'Ausgaben gesamt',
    admin_user_default: 'Benutzer',
    admin_user_email_missing: 'Keine E-Mail',
    delete_account_section: 'Sonstiges',
    delete_account_permanently: 'Konto löschen',
    shared_template_label: "geteilte Vorlage",
    shared_template_desc: "Mit dieser Vorlage passen Sie Ihre Bilder schnell an.",
    shared_template_cta: "Vorlage hinzufügen",
    shared_template_not_found: "Vorlage nicht gefunden",
    shared_template_invalid: "Dieser Link scheint ungültig zu sein oder die Vorlage wurde entfernt.",
    back_to_home: "Zur Startseite",
    learn_more_expose: "Erfahre mehr über exposé",

    // AppNavbar selection count
    image_selected_singular: "Bild ausgewählt",
    images_selected_plural: "Bilder ausgewählt",

    // GenerationProgressRing
    untitled: "Ohne Titel",
    go_to_image: "Zum Bild",
    generate_more: "Mehr generieren",

    // SideSheet annotation chips
    annotations_label: "Anmerkungen",
    brush_label: "Pinsel",
    shape_rect: "Rechteck",
    shape_circle: "Kreis",
    shape_line: "Linie",
    stamp_label: "Stempel",
    add_reference: "Inspiration hinzufügen",
    quality_highest: "Höchste Qualität",
    quality_faster: "Schneller & günstiger",
    presets_label: "Vorlagen",
    no_entries: "Keine Einträge",
    edit_presets: "Vorlagen bearbeiten",

    // CropModal
    crop_title: "Zuschneiden",
    crop_subtitle: "Passe den Bildausschnitt an.",

    // FeedPage
    select_images_to_edit: "Bilder zum Bearbeiten auswählen",
    n_download: "{{n}} herunterladen",

    // SettingsModal
    storage_section: "Speicher",
    images_label: "Bilder",
    notifications_label: "Benachrichtigungen",
    notifications_active: "Wird nach jeder Generierung ausgelöst",
    notifications_blocked: "Im Browser gesperrt",
    notifications_disabled: "Aktuell deaktiviert",
    notifications_ask: "Erlaubnis wird beim Aktivieren angefragt",
    notifications_allow_in_browser: "Benachrichtigungen in den Browser-Einstellungen erlauben",
    // SettingsModal — extended
    settings_model_label: "Standardmodell",
    settings_quality_label: "Qualität",
    settings_change_password: "Passwort ändern",
    settings_set_password: "Passwort festlegen (optional)",
    notifications_browser_label: "Browser-Benachrichtigungen",
    notifications_browser_desc: "Nach abgeschlossenen Generierungen",
    notification_generation_done: "exposé · Ihr Bild ist fertig",
    model_nb2_desc: "Schneller",
    model_pro_desc: "Höchste Qualität",

    // DetailPage
    open_editing_panel: "Bearbeitungspanel öffnen",

    // BrushTab tool labels
    tool_text: "Text",
    tool_shape: "Form",
    tool_circle: "Kreis",
    tool_line: "Linie",
    tool_brush: "Pinsel",
    clear_all_brush: "Pinselstriche löschen",
    tooltip_tool_text: "Text",
    tooltip_tool_shapes: "Formen",
    tooltip_tool_brush: "Pinsel",

    // ObjectsTab
    manage_stickers: "Sticker verwalten",
    add_new_sticker: "Neuen Sticker hinzufügen",
    no_stickers_yet: "Noch keine Sticker vorhanden.",
    create_first_sticker: "Ersten Sticker erstellen",
    edit_stickers_btn: "Sticker bearbeiten",
    sticker_create_tooltip: "Sticker erstellen",

    // ImageInfoModal
    copy_prompt_tooltip: "Prompt kopieren",

    // PromptTab
    select_model: "Modell auswählen",
    please_enter_prompt: "Bitte einen Prompt eingeben.",
    image_still_uploading: "Bild wird noch hochgeladen — gleich nochmal probieren.",
    reference_image_btn: "Referenzbild",
    annotation_instruction: "Setze die Anmerkungen im Bild fotorealistisch um.",
    reference_image_instruction: "Nutze dieses Bild als Inspiration ..",
    clear_all_annotations: "Alle Anmerkungen löschen",
    clear_all_annotations_desc: "Möchtest du wirklich alle Anmerkungen aus dem Canvas entfernen? Diese Aktion kann nicht rückgängig gemacht werden.",

    // AppNavbar
    nav_create: "Neu",
    nav_create_tooltip: "Bild hochladen oder generieren",
    nav_generate_new: "Neues Bild generieren",
    nav_upload: "Hochladen",
    nav_menu: "Menü",
    nav_start: "Start",
    nav_settings: "Einstellungen",
    nav_select: "Bilder auswählen",
    nav_gallery: "Galerie",
    nav_contact: "Feedback",
    nav_sign_out: "Abmelden",
    nav_logout: "Ausloggen",
    nav_download: "Herunterladen",
    nav_more: "Mehr",
    nav_sidebar: "Seitenleiste",
    nav_edit: "Bearbeiten",
    nav_delete: "Löschen",
    nav_info: "Info",

    // PresetLibrary
    presets_recent: "Zuletzt",
    presets_recently_used: "Zuletzt verwendet",
    presets_options: "Optionen",
    presets_edit_tooltip: "Vorlage bearbeiten",
    preset_edit_presets: "Vorlagen bearbeiten",
    no_entries_available: "Noch keine Einträge vorhanden.",

    // Home Page
    home_hero_cta: "Projekt starten",
    home_hero_headline: "bereit für next-gen creation?",
    home_section_iterative_title: "iterativ und parallel arbeiten",
    home_section_iterative_desc: "Ganze Bildstrecken gleichzeitig generieren, vergleichen und perfektionieren. Powered by Googles Nano Banana 2, dem besten KI-Bildmodell.",
    home_section_templates_title: "smarte vorlagen",
    home_section_templates_desc: "Prompt once, scale forever. Wählen oder erstellen Sie Ihre eigenen Prompt-Vorlagen für perfekt konsistente Ergebnisse in Serie.",
    home_section_visual_title: "Visual Prompting",
    home_section_visual_desc: "Sagen Sie der KI nicht nur was, sondern zeigen Sie ihr exakt wo.",
    home_mockup_prompt_hint: "Inszeniere das Bild neu indem du die Jahreszeit anpasst",
    home_mockup_season: "Jahreszeit",
    home_mockup_time: "Uhrzeit",
    home_mockup_generate: "Generieren",
    home_mockup_templates: "Vorlagen",
    home_mockup_note: "Anmerkung",
    home_mockup_ref: "Referenzbild",
    home_spring: "Frühling",
    home_summer: "Sommer",
    home_winter: "Winter",
    home_morning: "Morgen",
    home_noon: "Mittag",
    home_afternoon: "Nachmittag",
    home_golden_hour: "Golden Hour",
    home_blue_hour: "Blue Hour",
    home_kitchen: "Küche",
    home_table: "Esstisch",
    home_sofa: "Sofa",

    // AuthModal
    auth_or: "oder",

    // ManagePresetsModal / ShareTemplateModal
    preset_select_placeholder: "Vorlage auswählen oder neue erstellen",

    // Share Modal
    share_title: "Vorlage teilen",
    share_ready_desc: "Deine Vorlage ist bereit zum Teilen – einfach Link kopieren und weiterschicken",
    share_unsaved_desc: "Bitte speichere deine Änderungen zuerst, um einen Link zu generieren.",
    share_placeholder_unsaved: "Link wird nach dem Speichern generiert...",
    share_copy_btn: "Link kopieren",
    share_copied_btn: "Link kopiert",
    share_error_copy: "Fehler beim Kopieren",
    share_error_save_first: "Bitte speichere die Vorlage zuerst",
    share_toast_copied: "Link kopiert",
    share_btn: "Teilen",
  }
};
