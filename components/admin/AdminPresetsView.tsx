...
import { Search, Plus, Pen, Trash2, Tag, ChevronRight, Bookmark, GripVertical } from 'lucide-react';
...
import { DEFAULT_TEMPLATES, PRESET_TAGS_DATA } from '../../data/promptTemplates';
...
export const AdminPresetsView: React.FC<AdminPresetsViewProps> = ({ t }) => {
  const [globalPresets, setGlobalPresets] = useState<PromptTemplate[]>(DEFAULT_TEMPLATES);
  ...
  const [availableTags, setAvailableTags] = useState<PresetTag[]>(PRESET_TAGS_DATA);
  const [selectedTagId, setSelectedTagId] = useState<string | null>(null);

  // Drag State
  const [draggedPresetId, setDraggedPresetId] = useState<string | null>(null);

  const handleSavePresets = (data: { title: string; prompt: string; tags: string[]; controls: PresetControl[]; lang: 'de'|'en' }[]) => {
      setGlobalPresets(prev => {
          let updated = [...prev];
          
          data.forEach(item => {
              if (editingPreset && editingPreset.lang === item.lang) {
                  updated = updated.map(p => p.id === editingPreset.id ? { ...p, ...item } : p);
              } else {
                  updated.push({
                      id: generateId(),
                      ...item,
  ...
  const handleTogglePin = (id: string) => {
      setGlobalPresets(prev => prev.map(p => p.id === id ? { ...p, isPinned: !p.isPinned } : p));
  };
